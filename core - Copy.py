...
JAVA_ZXING_LOCATION1 = os.path.join(SERVER_LOCATION, "misc", "labeldesigner", "jclasses", "javase-3.2.1.jar")
JAVA_ZXING_LOCATION2 = os.path.join(SERVER_LOCATION, "misc", "labeldesigner", "jclasses", "core-3.2.1.jar")
print JAVA_ZXING_LOCATION1
print JAVA_ZXING_LOCATION2
print getDefaultJVMPath()
if __name__ == "__main__":
    startJVM(getDefaultJVMPath(), "-ea", ("-Djava.class.path=%s;%s" %  (JAVA_ZXING_LOCATION1, JAVA_ZXING_LOCATION2)))
print "g"
...
class Labeler(QtGui.QApplication):
    def __init__(self, *args, **kwargs):
        ...
        self.objectTypes = {"Text":LabelerTextItem, "Barcode":LabelerBarcodeItem, "Image":LabelerImageItem}
        ...
        self.dbEngine = create_engine("postgresql://labeldesigner:labelmaker666@pleqol01:5432/labelmaker")
        ...
        self.headerRE = re.compile('(\{.*?\})(\[.*?\])*') # New Style RE, matches {*}[*]
        ...
        self.connect(self.ui.clearData, QtCore.SIGNAL('clicked()'), self.clear_dataset)
        ...
        self.connect(self.ui.filterEdit, QtCore.SIGNAL('textChanged(QString)'), self.refresh_layout_list)
        self.connect(self.ui.clearFilter, QtCore.SIGNAL('clicked()'), self.ui.filterEdit.clear)
        ...
        self.connect(self.ui.averyRadio, QtCore.SIGNAL('clicked()'), self.refresh_printer_selection)
        self.connect(self.ui.zebraRadio, QtCore.SIGNAL('clicked()'), self.refresh_printer_selection)
        ...
        self.currentDirectory = '\\\\pleqol01\\data\\%s\\%s' % (thisYear, thisMonth)
    ...
    def refresh_printer_selection(self):
        ...
        printerType = unicode(self.ui.printerSelectGroup.checkedButton().text()).lower()
        ...
        for i in printers:
            name = i.printerName()
            if printerType in unicode(name).lower():
                defaultPrinter = name
    ...
    def create_carton_labels(self, accepted):
        ...
        if accepted:
            ...
            for i in range(cartons):
                row = []
                row.append(unicode(total))
                
                if i+1 <> cartons:
                    row.append(unicode(cartonQuantity))
                else:
                    ...
                    if val:
                        ...
                        row.append(unicode(cartonQuantity))
                row.append(unicode(i+1))
                row.append(unicode(cartons))
            ...
            if logo:
                logoText = self.add_text((45 + (largeLabel*13), 5), "relmm", select=False)
    ...
    def remove_layout(self):
        ...
        if layoutItem <> None:
            ret = QtGui.QMessageBox.warning(self.MainWindow, 'Remove Layout', 
                                     'Are you sure you would like to remove the \'%s\' layout?' % unicode(layoutItem.text()), 
                                     QtGui.QMessageBox.Yes|QtGui.QMessageBox.No, QtGui.QMessageBox.No )
            if ret == QtGui.QMessageBox.Yes:
                session = self.sessionMaker()
                q = session.query(tables.Layout).filter_by(name=unicode(layoutItem.text())).first()
                if q:
    ...
    def save_layout_to_db(self, layout):
        ...
        session.commit()
        for objName, objType, props in layout.objectList:
            obj = tables.LayoutObject(newLayout.id, unicode(objName), objType)
            session.add(obj)
            session.commit()
    ...
    def rename_layout(self):
        ...
        oldName = unicode(selected[0].text())
    ...
    def save_layout(self):
        ...
        while unicode(text).strip() == "":
            text, ok = QtGui.QInputDialog.getText(self.MainWindow, "Name of Layout", question, QtGui.QLineEdit.Normal, current)
            if ok and unicode(text).strip() <> "":
                layoutName = unicode(text).strip()
                # OK, Valid Name
                permit = unicode(self.ui.permitEntry.text())
                returnAddress = unicode(self.ui.returnAddress.toPlainText())
                ...
                for obj in self.objectCollection:
                    ...
                    for name, prop in obj.propNames.items():
                        ...
                        props.append((name, unicode(QtCore.QVariant(prop.get_value()).toString())))
            ...
            elif ok and unicode(text).strip() == "":
    ...
    def refresh_layout_list(self):
        ...
        filtered = []
        filterText = unicode(self.ui.filterEdit.text()).lower()
        if filterText <> "":
            for layout in layouts:
                if filterText in layout.lower():
        ...
        for layout, layoutobject, objectproperty in results:
            ...
            if layoutobject <> None:
                if objName <> layoutobject.name:
                    ...
                    if objName <> None:
                        if props <> None:
                            props.sort(key=lambda x: self.objectTypes[unicode(objType)].propLoadOrder.index(x[0]))
    ...
    def load_layout_from_conf(self, item):
        ...
        for objectName in self.settings.childGroups():
            ...
            objType = unicode(self.settings.value("type").toString())
            
            props = []
            for propName in self.settings.childKeys():
                if unicode(propName) == "type":
                    continue
    ...
    def set_layout(self, layout):
        ...
        if "<html>" in unicode(layout.returnAddress).lower(): 
        ...
        for objectName, objType, props in layout.objectList:
            ...
            obj = self.objectTypes[unicode(objType)](objectName)
            
            self.add_object(obj)
            for propName, propVal in props:
                if unicode(propName) == "type":
                    continue
                obj.propNames[unicode(propName)].set_value(QtCore.QVariant(propVal))
    ...
    def permit_number_changed(self, text):
        ...
        self.labelView.set_permit_number(unicode(text))
    ...
    def open_file(self):
        ...
        filename = unicode(QtGui.QFileDialog.getOpenFileName(self.MainWindow, "Select File", self.currentDirectory))
        if filename <> "":
    ...
    def setup_data(self):
        ...
        if len(self.dataSet) == 0:
            ...
            for i in self.headers:
                record = unicode(self.dataSet[self.ui.previewRecord.value()-1][i])
    ...
    def load_xls(self, filename):
        ...
        if result == True:
            ...
            for i in range(sheet.nrows):
                ...
                for cell in row:
                    if cell.ctype == 3:
                        celldate = xlrd.xldate_as_tuple(cell.value, xlfile.datemode)
                        if cell.xf_index == None:
                            celldate = datetime.datetime(*celldate).strftime("%d %B %Y")
                        ...
                        val = int(cell.value)
                        if val == cell.value:
                            newrow.append(unicode(int(cell.value)))
                        else:
                            newrow.append(unicode(cell.value))
                    else:
    ...
    def field_check(self):
        ...
        for obj in self.objectCollection:
            ...
            text = unicode(obj.get_merge_text())
            matches = self.headerRE.findall(text)
        ...
        if not headersMatched:
            if fieldMessage:
                self.log_message("Error, the following fields could not be found: " + fieldMessage[1:-1], "error")
            if substringMessage:
                self.log_message("Error, the substring formatting is incorrect for: " + substringMessage[1:-1], "error")
    ...
    def make_labels(self, method="PDF"):
        ...
        if method == "PRINT":
            ...
            if not result:
                res = QtGui.QMessageBox.question(self.MainWindow, "Cancel labels", "Did you want to cancel the labels?", QtGui.QMessageBox.Yes|QtGui.QMessageBox.No)
        ...
        if self.ui.individualPdfs.isChecked() and method == "PDF":
            ...
            zipFilename = os.path.splitext(self.currentFile)[0] + ("_%d_labels.zip" % length)
        ...
        for row in self.mergeDataSet:
            if self.ui.individualPdfs.isChecked() and method == "PDF":
                ...
                outputName = os.path.split(self.currentFile)[0] + "\\" + self.merge_value(self.ui.pdfNameFormat.text(), row)
            ...
            if self.ui.copyUseField.isChecked():
                ...
                copies = int(row[unicode(self.ui.copyField.currentText())])
        ...
        if method == "PRINT":
            if rollLabels:
                labelCount = len(self.dataSet) / 4000.0
                
                if labelCount <> int(labelCount):
                ...
                for i in range(labelCount):
                    text = "%s\n%s\nRoll %d of %d" % (self.rollLabelUi.jobNo.text(), 
                                                      self.rollLabelUi.jobName.text(), i+1, labelCount)
                    #x = QtGui.QPixmap(self.currentPageSize[0], self.currentPageSize[1])
                    #p = QtGui.QPainter(x)
                    #rect = QtCore.QRectF(self.currentPageSize[0] * 0.05, self.currentPageSize[1] * 0.05, 
                    #                      self.currentPageSize[0] * 0.9, self.currentPageSize[1] * 0.9)
                    ...
                    #painter.drawText(rect, QtCore.Qt.AlignVCenter|QtCore.Qt.AlignHCenter, text)
                    ...
                    label.setMinimumHeight(self.currentPageSize[1]*self.dpmm[1])
                    label.setMaximumHeight(self.currentPageSize[1]*self.dpmm[1])
                    label.setMinimumWidth(self.currentPageSize[0]*self.dpmm[0])
                    label.setMaximumWidth(self.currentPageSize[0]*self.dpmm[0])
