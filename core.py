...
class Labeler(QtGui.QApplication):
    ...
    def remove_layout(self):
        ...
        if layoutItem <> None:
            ret = QtGui.QMessageBox.warning(self.MainWindow, 'Remove Layout',
                                            'Are you sure you would like to remove the \'%s\' layout?' % unicode(
                                                layoutItem.data(QtCore.Qt.UserRole).toString()),
                                            QtGui.QMessageBox.Yes | QtGui.QMessageBox.No, QtGui.QMessageBox.No)
            if ret == QtGui.QMessageBox.Yes:
                session = self.sessionMaker()
                q = session.query(tables.Layout).filter_by(name=unicode(
    ...
    def rename_layout(self):
        ...
        oldName = unicode(selected[0].data(QtCore.Qt.UserRole).toString())
            ...
            if ok and unicode(text).strip() <> "":
                ...
                for obj in self.objectCollection:
                    ...
                    for name, prop in obj.propNames.items():
                        ...
                        props.append(
                            (name, unicode(QtCore.QVariant(prop.get_value()).toString())))
    ...
    def load_layout_from_db(self, item):
        ...
        name = unicode(item.data(QtCore.Qt.UserRole).toString())
        ...
        for layout, layoutobject, objectproperty in results:
            ...
            if layoutobject <> None:
                if objName <> layoutobject.name:
                    ...
                    if objName <> None:
                        if props <> None:
                            props.sort(key=lambda x: self.objectTypes[unicode(
    ...
    def set_layout(self, layout):
        ...
        for objectName, objType, props in layout.objectList:
            ...
            for propName, propVal in props:
                if unicode(propName) == "type":
                    continue
                obj.propNames[unicode(propName)].set_value(
        ...
        if currentItem:
            ...
            if obj.objectType == "Text":
                ...
                for i in range(self.ui.headerList.rowCount()):
                    x.append(
                        "{" + unicode(self.ui.headerList.item(i, 0).text()) + "}")
